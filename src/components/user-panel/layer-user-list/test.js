describe('layer-user-list', function() {
  ...
  beforeEach(function(done) {
    ...
    el = document.createElement('layer-user-list');
  ...
  describe('Event Handling', function() {
    it("Should call onUserSelected when child triggers layer-user-item-selected", function() {
      var spy = jasmine.createSpy('callback');
      el.onUserSelected = spy;
      el.firstChild.trigger('layer-user-item-selected', {user: query.data[1]});
      expect(spy).toHaveBeenCalledWith(jasmine.any(CustomEvent));
    });

    it("Should call onUserDeselected when child triggers layer-user-item-deselected", function() {
      var spy = jasmine.createSpy('callback');
      el.selectedUsers = [query.data[1]];
      el.onUserDeselected = spy;
      el.firstChild.trigger('layer-user-item-deselected', {user: query.data[1]});
      expect(spy).toHaveBeenCalledWith(jasmine.any(CustomEvent));
    });
  });

  describe("The selectedUsers property", function() {
    it("Should mark all specified users as selected", function() {
      el.childNodes[10].selected = true;
      el.childNodes[11].selected = true;
      debugger;
      el.selectedUsers = [query.data[1], query.data[2]];
    ...
    it("Should clear all selected users", function() {
      el.childNodes[10].selected = true;
      el.childNodes[11].selected = true;
      el.selectedUsers = null;
  ...
  describe("The created() method", function() {
    it("Should call updateQuery if there is a queryId passed into the innerHTML", function() {
      testRoot.innerHTML = '<layer-user-list query-id="' + query.id + '" app-id="' + client.appId + '"></layer-user-list>';
      CustomElements.takeRecords();
    ...
    it("Should call render", function() {
      testRoot.innerHTML = '<layer-user-list></layer-user-list>';
      CustomElements.takeRecords();
      var el = testRoot.firstChild;
      expect(el.nodes.loadIndicator).toEqual(jasmine.any(HTMLElement));
    });

    it("Should wire up handleUserSelect and handleUserDeselect", function() {
      var selectSpy = jasmine.createSpy('select');
      var deselectSpy = jasmine.createSpy('deselect');
      el.addEventListener('layer-user-selected', selectSpy);
      el.addEventListener('layer-user-deselected', deselectSpy);
      ...
      el.selectedUsers.push(query.data[1]);
      el.firstChild.trigger('layer-user-item-deselected', {user: query.data[1]});
  ...
  describe("The handleUserSelect() method", function() {
    it("Should add item to selectedUsers", function() {
      expect(el.selectedUsers).toEqual([]);
      el.firstChild.trigger('layer-user-item-selected', {user: query.data[1]});
      expect(el.selectedUsers).toEqual([query.data[1]]);
    });

    it("Should call evt.preventDefault if evt.preventDefault and leave selectedUsers unchanged", function() {
      el.addEventListener('layer-user-selected', function(evt) {
    ...
    it("Should do nothing if already in the list", function() {
      ...
      el.addEventListener('layer-user-selected', spy);
      el.handleUserSelect({
        detail: {
          user: query.data[1]
        },
  ...
  describe("The handleUserDeselect() method", function() {
    it("Should remove item from selectedUsers", function() {
      el.selectedUsers = [query.data[1]];
      el.firstChild.trigger('layer-user-item-deselected', {user: query.data[1]});
      expect(el.selectedUsers).toEqual([]);
    });
    ...
    it("Should call evt.preventDefault if evt.preventDefault and leave selectedUsers unchanged", function() {
      el.selectedUsers = [query.data[1]];
      el.addEventListener('layer-user-deselected', function(evt) {
        expect(evt.preventDefault.calls).toBe(undefined); // Not the spy
        evt.preventDefault();
      });
      var preventDefaultSpy = jasmine.createSpy('preventDefault');
      el.handleUserDeselect({
        detail: {
          user: query.data[1]
        },
        stopPropagation: jasmine.createSpy('stopPropagation'),
    ...
    it("Should do nothing if not in the list", function() {
      ...
      el.addEventListener('layer-user-deselected', spy);
      el.handleUserDeselect({
        detail: {
          user: query.data[2]
        },
  ...
  describe("The generateItem() method", function() {
    it("Should return a layer-user-item with a user setup", function() {
      var result = el.generateItem(query.data[1]);
      expect(result.tagName).toEqual('LAYER-USER-ITEM');
  ...
  describe("The rerender() method", function() {
    ...
    it("Should remove any removed users from selectedUsers", function() {
      el.selectedUsers = [query.data[2], query.data[1], query.data[0]];
      el.rerender({
        type: 'remove',
        target: query.data[1]
      });
      expect(el.selectedUsers).toEqual([query.data[2], query.data[0]]);
    });

    it("Should reset selectedUsers when data is reset", function() {
      el.selectedUsers = [query.data[2], query.data[1], query.data[0]];
      el.rerender({
        type: 'reset'
      });
  ...
  describe("The renderSelection() method", function() {
    it("Should select and deselect appropriately", function() {
      ...
      el.selectedUsers.pop();
      el.selectedUsers.pop();
      el.selectedUsers.push(query.data[5]);
      el.selectedUsers.push(query.data[6]);

      el.renderSelection();
      expect(el.childNodes[0].innerNode.classList.contains('layer-user-item-selected')).toBe(false);
      expect(el.childNodes[1].innerNode.classList.contains('layer-user-item-selected')).toBe(false);
      expect(el.childNodes[5].innerNode.classList.contains('layer-user-item-selected')).toBe(true);
      expect(el.childNodes[6].innerNode.classList.contains('layer-user-item-selected')).toBe(true);
    });
  });
  ...
  describe("Mixin list", function() {
    describe("Properties", function() {
      it("Should set the query given a queryId and then a client", function() {
        var el = document.createElement('layer-user-list');
        el.queryId = query.id;
    ...
    describe("The generateFragmentItem() method", function() {
      it("Should create a widget via generateItem and add it to the fragment", function() {
        ...
        expect(fragment.childNodes[0].tagName).toEqual('LAYER-USER-ITEM');
  ...
  describe("Mixin main-component", function() {
    describe("Common Properties", function() {
      ...
      it("Should setup the client from the app-id attribute", function() {
        testRoot.innerHTML = '<layer-user-list app-id="' + client.appId + '"></layer-user-list>';
        CustomElements.takeRecords();
        var el = testRoot.firstChild;
        expect(el.client).toBe(client);
      });
